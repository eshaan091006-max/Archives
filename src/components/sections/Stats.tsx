...
export const Stats = () => {
  const stats = [
    { label: 'Total Events', value: '58+', color: 'text-[var(--color-text-highlight)]' },
    { label: 'Attendees', value: '30k+', color: 'text-[var(--color-text-main)]' },
    { label: 'Workforce', value: '1k+', color: 'text-[var(--color-text-main)]' },
    { label: 'Days', value: '3', color: 'text-[var(--color-text-main)]' },
  ];
          ...
          {stats.map((stat, i) => {
            return (
              <div key={i} className="flex flex-col items-center justify-center text-center group">
                <span
                  className={`text-4xl md:text-5xl font-['Boldonse'] mb-2 ${stat.color} transition-colors duration-500`}
                >
                  {stat.value}
                </span>
