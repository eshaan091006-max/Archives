...
export const Spotlight = () => {
  ...
  const [bodiesData, setBodiesData] = useState<
    Array<{ id: number; type: 'shard' | 'dust'; size: number; x: number; y: number; color: string }>
  >([]);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const body = Matter.Bodies.rectangle(Math.random() * w, Math.random() * h, size, size, {
        restitution: 1.2,
        frictionAir: 0.02,
      });
      bodies.push(body);
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const body = Matter.Bodies.circle(Math.random() * w, Math.random() * h, size, {
        restitution: 0.9,
        frictionAir: 0.05,
      });
      bodies.push(body);
    ...
    >
      ...
      >
        ...
        <div className="w-full h-full flex flex-col items-center justify-center text-center p-8 opacity-30 bg-[var(--color-bg-secondary)]/20">
          <h2 className="text-sm font-['Inter'] font-bold tracking-[0.5em] uppercase mb-4 text-[var(--color-text-main)]">
            System Notification
          </h2>
